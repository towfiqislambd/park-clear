    ...
    <div className="w-[20%] plan-card">
      ...
      <div className="plan-header">
        ...
        <PrimaryButton
          ...
          className={`!text-xs 3xl:text-sm justify-center mt-5 3xl:mt-7 ${item.price === "free"
      ...
      <ul>
        {
          item?.featureStatus?.map((status, index) => (
            <li key={index} className="py-5 px-5 3xl:px-8 min-h-20 max-h-20 flex items-center justify-center border-b border-default-border first:border-t last:border-none">{status ? <img className="h-6 w-6" src={checkCircle} alt="checkCircle" /> : <img className="h-6 w-6" src={closeCircle} alt="closeCircle" />} </li>
          ))
        }
      </ul>
    </div>
