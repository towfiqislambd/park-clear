...
const Navbar = () => {
  ...
  return (
    <header className="py-4 md:py-5 3xl:py-7 shadow-header-shadow sticky top-0 left-0 w-full z-[10] bg-white" data-aos="fade-down">
      <Container>
        ...
        <div
          ...
          className={`fixed inset-0 bg-black/30 backdrop-blur-sm max-h-screen min-h-screen transition-opacity duration-300 2xl:hidden z-[999] ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        </div>

        {/* Mobile Sidebar */}
        <div className={`${isOpen ? 'translate-x-0' : '-translate-x-full'} duration-500 transition-transform fixed top-0 z-[9999] left-0 bg-white p-5 lg:p-7 shadow-lg overflow-y-auto  border-r max-h-screen min-h-screen w-[250px] lg:w-[270px] 2xl:hidden`}
        >
          {/* logo */}
